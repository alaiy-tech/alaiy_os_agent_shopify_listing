...
It may instead (or additionally) contain raw fields directly, e.g. `title`, `description`, `price` — use those if present. If an `item_code` is present, treat the stored Item as the source of truth.
