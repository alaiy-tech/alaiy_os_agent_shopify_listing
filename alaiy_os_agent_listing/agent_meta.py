...
its images, RETURNS a JSON object, and (via the save_listing tool) persists
that listing into its own Enriched Listing DocType in "Needs Review" status for
admin review. It does not modify the Item itself or publish to Shopify (that is
	...
	"tools": [
		...
		},
		{
			...
			"description": (
				...
				"item_code run — the tool reads the Item's own photo) and "
			...
			"parameters_schema": {
				...
				"properties": {
					...
					"item_code": {
						"type": "string",
						"description": (
							"The ERPNext Item being enriched. The tool reads this "
							"Item's own original photo and edits it; if the Item "
							"has no photo, nothing is generated."
						),
					},
					"generate_images": {
						"type": "boolean",
		...
		},
		{
			...
			"description": (
				...
				"input), since the record is keyed to an ERPNext Item."
			),
			...
			"parameters_schema": {
				...
				"properties": {
					"item_code": {
						"type": "string",
						"description": "The ERPNext Item code this listing is for (upsert key).",
					},
