	...
	"tools": [
		...
		},
		{
			...
			"description": (
				...
				"that order. Always pass `reference_image_url` when a real photo "
				"of the product exists (the input's `image_url`, or another photo "
				"URL you viewed via view_image) — every shot is then generated by "
				"editing that actual photo, so they stay visually grounded in the "
				"real piece instead of a generic imagined rendering. Each brief "
				"must be a detailed, self-contained prompt describing the exact "
				"product (material, style, gemstone, setting) and the shot itself "
				"(framing, background, lighting) — the image model has no memory "
				"of the conversation, only that brief (and the reference image, if "
				"given). Returns {images: [{kind, brief, url}, ...]}; copy that "
				"list verbatim into the final `images` array. If image "
			...
			"parameters_schema": {
				...
				"properties": {
					...
					"reference_image_url": {
						"type": "string",
						"description": (
							"URL of a real photo of this exact product. When given, "
							"every shot is generated by editing this photo instead of "
