...
8. **Fill the attributes.** Work down the list one attribute at a time, in this order:
   1. **Is it in `published_attributes`?** Then the product already answers it. Repeat that value (normalised, if its spelling is untidy) and move on — do not re-derive it, and never contradict it without saying why in `notes`. This is a value a shopper can see on the live product; your output replaces it, so replacing it with a guess or a blank is a regression.
   ...
   5. Only when all four fail does the attribute go in `needs_review`.
