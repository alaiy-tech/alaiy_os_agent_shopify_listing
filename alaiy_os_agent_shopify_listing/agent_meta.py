	...
	return {
		...
		"generate_product_images": {
			"description": (
				...
				"{images: [{kind, brief, url}, ...]}; copy that list into the final "
				"`images` array, but DROP each entry's `brief` — it is already stored "
				"against the image, the final schema has no place for it, and repeating "
				"it there would be rejected. EXPECT url TO BE null: the shots are rendered "
