...
def tool_catalog(output_schema):
	return {
		...
		"generate_product_images": {
			"description": (
				"Retouch the product's existing photos: each one comes back cleaner — "
				"even lighting, true colour, no glare or dust, a tidy background — "
				"and otherwise unchanged. It does NOT compose new shots, and you do "
				"NOT describe the imagery you want: there are no briefs and no shot "
				"types, because the retouch instruction is fixed in the tool and the "
				"photograph itself is the only description of the product that may "
				"matter. Call it ONCE for the whole product; it processes every photo "
				"— the listing's own AND each enabled variant's — in one go. Pass "
				"`item_code` (for an item_code run — the tool reads the listing's own "
				"photos) and `generate_images` copied verbatim from the input "
				"(default false). For a URL-only product, pass the real photo urls as "
				"`image_urls` instead. Whether anything is produced is decided by the "
				"tool, NOT by you: it enhances ONLY when the product already has "
				"photos AND generate_images is true, and it never creates imagery "
				"from scratch. Returns {images: [{source_url, item_variant, url, "
				"note}, ...]}; copy that list into the final `images` array VERBATIM, "
				"including each entry's item_variant. EXPECT url TO BE null: the "
				"photos are enhanced in the background after this run finishes and "
				...
				"images in needs_review, and do NOT describe them as missing or "
				"failed anywhere in your output. Some entries may come back with a "
				"real url and a note saying they were reused from an earlier run; "
				"copy those verbatim too. If it returns an empty list with a note "
				"(the product has no photos, or the toggle is off), that is also "
			...
			"input_option": {
				...
				"label": "Enrich images",
				"description": (
					"Retouch this listing's existing photos — and its variants' — for "
					"cleaner lighting, colour and background. Costs money per photo; "
					"turn off for a faster, text-only enrichment."
				),
				"default": 0,
			},
			"parameters_schema": {
				...
				"properties": {
					"item_code": {
						...
						"description": (
							...
							"and its variants' photos; if it has none, nothing is done."
						),
					},
					"generate_images": {
						"type": "boolean",
						"description": (
							...
							"is true AND the product already has photos."
						),
					},
					"image_urls": {
						"type": "array",
						"items": {"type": "string"},
						"description": (
							"Only for a URL-only product with no item_code: the urls of "
							"that product's real photos. Ignored when item_code "
							"resolves to a listing with photos."
						),
					},
				},
			},
